...
def main(args):
    ...
    demo_loss_list = []
    with torch.no_grad():
        for i_iter_demo, (_, demo_vox_label, demo_grid, demo_pt_labs, demo_pt_fea, index) in enumerate(
                demo_dataset_loader):
            ...
            for count, i_demo_grid in enumerate(demo_grid):
                ...
                outputPath = save_dir + os.path.basename(demo_dataset_loader.dataset.point_cloud_dataset.im_idx[index[0]]).split(".")[0] + '.label'
...
if __name__ == '__main__':
    ...
    labels_yaml = yaml.unsafe_load(open('config/label_mapping/semantic-kitti.yaml', 'rt'))
    label_name_dict = labels_yaml['labels']
    json.dump({"kitti_label":label_name_dict}, open(args.output_dir + "/meta.json", 'wt'))
